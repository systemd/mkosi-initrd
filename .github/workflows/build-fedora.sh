...
for phase in "${PHASES[@]}"; do
    case "$phase" in
        ...
        INITRD_ISCSI_FULLBOOT)
            ...
            iscsi_cmdline="root=LABEL=root ip=dhcp netroot=iscsi:10.10.10.1::::$target_name"
            timeout -k 10 5m qemu-kvm -m 1024 -smp "$(nproc)" -nographic -nic bridge,br=initrd0 \
                                      -initrd "mkosi.output/initrd_$KVER.cpio.zstd" \
                                      -kernel "/usr/lib/modules/$KVER/vmlinuz" \
                                      -append "$iscsi_cmdline rd.debug $SYSTEMD_LOG_OPTS console=ttyS0 systemd.unit=systemd-poweroff.service systemd.default_timeout_start_sec=240"

            # Cleanup
            tgtadm --lld iscsi --op delete --mode target --tid=1
            pkill -INT tgtd
            pkill dnsmasq
            rm -fr _rootfs mkosi.output
            ;;
        SYSEXT)
            rm -fr mkosi.output
            mkdir mkosi.output
